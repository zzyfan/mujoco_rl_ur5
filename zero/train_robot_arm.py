...
LOG_DIR = Path("logs") / "td3"
MODEL_DIR = Path("models") / "td3"
...
FINAL_MODEL_PATH = MODEL_DIR / "td3_robot_arm_final"
...
class ManualInterruptCallback(BaseCallback):
    ...
    def save_model(self):
        if self.model is not None:
            ...
            self.model.save(str(interrupted_dir / "td3_robot_arm_interrupted"))
...
class EpisodeSummaryCallback(BaseCallback):
    """
    训练日志精简回调：
    1. 不逐 step 打印，避免打断 progress bar。
    2. 仅按固定“回合窗口”输出聚合统计，方便看收敛趋势。
    """
    ...
    def _flush_window(self):
        ...
        success_rate = self.window_successes / episodes
        collision_rate = self.window_collisions / episodes
        return_mean = self.window_return_sum / episodes
        final_distance_mean = self.window_final_distance_sum / episodes
        min_distance_mean = self.window_min_distance_sum / episodes
        avg_speed_mean = self.window_avg_speed_sum / episodes
        top_reasons = ",".join(
            f"{name}:{count}" for name, count in self.window_reasons.most_common(3)
        )
        print(
            "[Train Summary] "
            ...
            f"ret_mean={return_mean:.2f} "
            f"dist_final_mean={final_distance_mean:.4f} "
            f"dist_min_mean={min_distance_mean:.4f} "
            f"speed_avg_mean={avg_speed_mean:.4f} "
...
def resolve_td3_hyperparams(_n_envs):
    """
    TD3 推荐参数（deterministic baseline）：
    - 与当前项目的稳定训练设置保持一致。
    - 训练频率采用 (16, "step") 并配套 16 次梯度更新。
    """
    return {
        "learning_rate": 3e-4,
...
def train_robot_arm(
    ...
    hparams = resolve_td3_hyperparams(n_envs)
    print(
        "[Train Config] "
        f"n_envs={n_envs} lr={hparams['learning_rate']} batch={hparams['batch_size']} "
        f"train_freq={hparams['train_freq']} grad_steps={hparams['gradient_steps']} "
        f"starts={hparams['learning_starts']} "
        f"eval_freq={eval_freq} eval_episodes={eval_episodes} "
        f"final_eval_episodes={final_eval_episodes}",
        flush=True,
    )
    n_actions = env.action_space.shape[-1]
    action_range = (env.action_space.high - env.action_space.low) / 2.0
    action_noise = NormalActionNoise(
        mean=np.zeros(n_actions),
        sigma=float(hparams["action_noise_scale"]) * action_range,
    )
    ...
    model = TD3(
        ...
        learning_starts=int(hparams["learning_starts"]),
        batch_size=int(hparams["batch_size"]),
        tau=float(hparams["tau"]),
        gamma=float(hparams["gamma"]),
        train_freq=hparams["train_freq"],
        gradient_steps=int(hparams["gradient_steps"]),
        policy_delay=int(hparams["policy_delay"]),
        target_policy_noise=float(hparams["target_policy_noise"]),
        target_noise_clip=float(hparams["target_noise_clip"]),
...
def test_robot_arm(model_path=FINAL_MODEL_PATH, normalize_path=VEC_NORMALIZE_PATH, num_episodes=10):
    ...
    model = TD3.load(str(resolved_model_path), env=env)
    episode_rewards = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-envs", type=int, default=12, help="Number of parallel environments for training")
