...
class SaveVecNormalizeCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_callback.best_mean_reward > self.best_mean_reward:
            ...
            vec_normalize_path = "./logs/sac/best_model"
...
class ManualInterruptCallback(BaseCallback):
    ...
    def save_model(self):
        ...
        if self.model is not None:
            # 创建保存目录
            os.makedirs("./models/sac/interrupted", exist_ok=True)
            
            # 保存模型
            self.model.save("./models/sac/interrupted/sac_robot_arm_interrupted")
            
            # 保存VecNormalize参数
            env = self.model.get_vec_normalize_env()
            if env is not None:
                env.save("./models/sac/interrupted/vec_normalize.pkl")
                
            print("已保存中断时的模型和参数到 ./models/sac/interrupted/")
...
def train_robot_arm():
    ...
    # 保存归一化环境和最终模型
    env.save("./models/sac/vec_normalize.pkl")
    model.save("./models/sac/sac_robot_arm_final")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="./models/sac/sac_robot_arm_final", 
                        help="Path to the model for testing")
    parser.add_argument("--normalize-path", type=str, default="./models/sac/vec_normalize.pkl",
