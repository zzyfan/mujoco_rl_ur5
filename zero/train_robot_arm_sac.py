#!/usr/bin/env python3
"""
使用Stable-Baselines3的SAC算法训练机械臂进行位置跟踪
"""
...
FINAL_MODEL_PATH = MODEL_DIR / "sac_robot_arm_final"
...
class ManualInterruptCallback(BaseCallback):
    ...
    def save_model(self):
        if self.model is not None:
            ...
            self.model.save(str(interrupted_dir / "sac_robot_arm_interrupted"))
...
def resolve_sac_hyperparams(_n_envs):
    """最终 SAC 推荐参数。"""
    return {
        "learning_rate": 3e-4,
...
def train_robot_arm(
    ...
    hparams = resolve_sac_hyperparams(n_envs)
    print(
        "[Train Config] "
        ...
        f"starts={hparams['learning_starts']} sde={hparams['use_sde']} "
    ...
    model = SAC(
        ...
        use_sde=bool(hparams["use_sde"]),
        sde_sample_freq=int(hparams["sde_sample_freq"]),
...
def test_robot_arm(model_path=FINAL_MODEL_PATH, normalize_path=VEC_NORMALIZE_PATH, num_episodes=10):
    ...
    model = SAC.load(str(resolved_model_path), env=env)
    episode_rewards = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or test robot arm with SAC")
