...
def _apply_zero_original_preset(args: TrainArgs) -> None:
    """在 zero_robotiq 场景下，按 zero 原始项目习惯自动补齐默认参数。"""
    ...
    if applied:
        print(f"已应用 zero 原始参数预设: {', '.join(applied)}")
...
def _make_env(args: TrainArgs, render_mode: str | None = None):
    ...
    if args.robot == "zero_robotiq":  # zero 机械臂 + Robotiq 夹爪配置分支。
        ...
        if args.fixed_target_y is None:
            cfg.fixed_target_y = float(np.clip(cfg.target_y_min + 0.65 * (cfg.target_y_max - cfg.target_y_min), cfg.target_y_min, cfg.target_y_max))
        if args.fixed_target_z is None:
            cfg.fixed_target_z = float(np.clip(cfg.target_z_min + 0.55 * (cfg.target_z_max - cfg.target_z_min), cfg.target_z_min, cfg.target_z_max))
    else:
    ...
    if args.resume:  # 继续训练模式。
        ...
        if not _path_exists_with_optional_zip(resume_model_path):
            ...
            if not args.resume_model_path and (
                ...
                print(f"当前分层路径不存在，已兼容回退到旧路径继续训练: {legacy_model_path}")
