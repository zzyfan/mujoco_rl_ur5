...
class TrainArgs:
    ...
    buffer_size: int = 3_000_000  # 回放池太小会更快覆盖旧轨迹，太大则恢复训练更占内存。
    gradient_steps: int = 4  # 配合 n_envs 一起看；并行越大，同样 gradient_steps 的样本利用率越低。
    learning_starts: int = 20000  # 经验池至少积累多少步后开始学习。
    action_noise_sigma: float = 0.25  # TD3 动作噪声标准差；动作空间改成标准化后同步收小。
    ...
    joint_position_delta_scale: float = 0.08  # `joint_position_delta` 模式下每步的关节目标增量。
    position_control_kp: float = 45.0  # 位置控制模式比例增益。
    position_control_kd: float = 3.0  # 位置控制模式阻尼增益。
    goal_conditioned: bool = False  # 是否切换成 goal-conditioned 观测结构。
    reward_mode: str = "dense"  # 奖励模式：`dense` 或 `sparse`。
    use_her: bool = False  # 是否在 classic 的 off-policy 算法里启用 HER。
    her_goal_selection_strategy: str = "future"  # HER relabel 策略。
    her_n_sampled_goal: int = 4  # HER 每条样本额外重标记的目标数。
...
class TrainingLogCallback(BaseCallback):
    ...
    def __init__(self, log_freq_timesteps: int, eval_callback: EvalCallback | None = None, verbose: int = 0):
        ...
        self.recent_runaways: deque[float] = deque(maxlen=20)
        self.recent_timeouts: deque[float] = deque(maxlen=20)
        self.recent_done_reasons: deque[str] = deque(maxlen=20)
    ...
    def _maybe_append_episode_stats(self) -> None:
        ...
        for done, info in zip(dones, infos):
            ...
            if "runaway" in info:
                self.recent_runaways.append(float(info["runaway"]))
            if "timeout" in info:
                self.recent_timeouts.append(float(info["timeout"]))
            if "done_reason" in info:
                self.recent_done_reasons.append(str(info["done_reason"]))
    ...
    def _on_step(self) -> bool:
        ...
        runaway_rate = self._mean_or_none(self.recent_runaways)
        timeout_rate = self._mean_or_none(self.recent_timeouts)
        ...
        if self.recent_done_reasons:
            ...
            parts.append(f"done_reasons={reason_summary}")
...
def _apply_zero_original_preset(args: TrainArgs) -> None:
    ...
    if args.algo == "td3" and abs(float(args.action_noise_sigma) - 0.25) < 1e-12:
...
def _apply_goal_conditioned_preset(args: TrainArgs) -> None:
    """把 goal-conditioned / HER 相关开关收成一套兼容配置。"""
    applied: list[str] = []
    if args.use_her:
        if args.algo not in {"sac", "td3"}:
            raise ValueError("HER 目前只在 classic 的 SAC / TD3 上启用。")
        if not args.goal_conditioned:
            args.goal_conditioned = True
            applied.append("goal_conditioned=True")
...
def _make_env(args: TrainArgs, render_mode: str | None = None):
    ...
    cfg = MujocoEnvConfig(  # 把训练参数映射成环境配置对象。
        ...
        success_threshold=args.success_threshold,  # 最终阶段成功判定阈值。
        stage1_success_threshold=args.stage1_success_threshold,  # 固定目标阶段成功阈值。
        stage2_success_threshold=args.stage2_success_threshold,  # 小范围随机阶段成功阈值。
        action_target_scale=args.action_target_scale,  # 标准化动作缩放比例。
        action_smoothing_alpha=args.action_smoothing_alpha,  # 动作滤波系数。
        controller_mode=args.controller_mode,  # 控制接口类型。
        joint_position_delta_scale=args.joint_position_delta_scale,  # position-delta 每步目标增量。
        position_control_kp=args.position_control_kp,  # 位置控制比例增益。
        position_control_kd=args.position_control_kd,  # 位置控制阻尼增益。
        goal_conditioned=args.goal_conditioned,  # 是否输出 goal-conditioned 观测。
        reward_mode=args.reward_mode,  # 奖励模式。
...
def _rebuild_replay_buffer_if_needed(model, env) -> None:
    ...
    if rb_class is HerReplayBuffer:
...
def _build_model(args: TrainArgs, env, device: str):
    ...
    her_goal_strategy = GoalSelectionStrategy[args.her_goal_selection_strategy.upper()]
    her_kwargs = dict(
        n_sampled_goal=max(int(args.her_n_sampled_goal), 1),
        goal_selection_strategy=her_goal_strategy,
        copy_info_dict=True,  # 让 compute_reward 能读取阶段性 success_threshold。
    )
    if args.algo == "td3":  # TD3 分支。
        ...
        return TD3(  # 返回 TD3 模型实例。
            policy_name,  # goal-conditioned 时切换到 MultiInputPolicy。
            env,  # 训练环境。
            ...
            replay_buffer_class=HerReplayBuffer if args.use_her else None,  # TD3+HER 是 robotics 常见强基线。
            replay_buffer_kwargs=her_kwargs if args.use_her else None,
    ...
    if args.algo == "sac":  # SAC 分支。
        ...
        return SAC(  # 返回 SAC 模型实例。
            ...
            replay_buffer_class=HerReplayBuffer if args.use_her else None,  # robotics 任务优先试 SAC+HER。
            replay_buffer_kwargs=her_kwargs if args.use_her else None,
            policy_kwargs=policy_kwargs,  # 网络配置。
        )
...
def train(args: TrainArgs):  # 训练主流程：排查训练问题优先顺着这里读
    ...
    _apply_goal_conditioned_preset(args)  # goal-conditioned/HER 先收成兼容配置。
...
def test(args: TrainArgs):  # 测试主流程：验收模型表现时会走这里
    ...
    _apply_goal_conditioned_preset(args)  # 保证测试时的观测结构和训练保持一致。
    register_env()  # 注册环境。
...
def parse_args() -> TrainArgs:  # 命令行解析入口：外部调参首先会影响这里
    ...
    p.add_argument("--buffer-size", type=int, default=3_000_000)  # SAC/TD3 回放池容量。
    ...
    p.add_argument("--action-noise-sigma", type=float, default=0.25)  # TD3 动作噪声。
    ...
    p.add_argument("--action-target-scale", type=float, default=0.6)  # 标准化动作到真实扭矩的缩放比例。
    p.add_argument("--action-smoothing-alpha", type=float, default=0.75)  # 动作低通滤波系数。
    p.add_argument("--controller-mode", choices=["torque", "joint_position_delta"], default="torque")  # 控制接口类型。
    p.add_argument("--joint-position-delta-scale", type=float, default=0.08)  # 位置增量控制时每步允许的关节目标增量。
    ...
    p.add_argument("--goal-conditioned", action="store_true")  # 输出 goal-conditioned 观测结构。
    p.add_argument("--reward-mode", choices=["dense", "sparse"], default="dense")  # 奖励模式。
    p.add_argument("--use-her", action="store_true")  # 在 SAC/TD3 上启用 HER。
    p.add_argument("--her-goal-selection-strategy", choices=["future", "final", "episode"], default="future")  # HER 目标重标记策略。
    p.add_argument("--her-n-sampled-goal", type=int, default=4)  # HER 每条样本额外采样多少个目标。
    ...
    return TrainArgs(  # 显式映射到 dataclass，避免隐藏字段错误。
        ...
        stage1_success_threshold=ns.stage1_success_threshold,  # 阶段 1 成功阈值。
        stage2_success_threshold=ns.stage2_success_threshold,  # 阶段 2 成功阈值。
        frame_skip=ns.frame_skip,  # frame-skip。
        action_target_scale=ns.action_target_scale,  # 标准化动作缩放比例。
        action_smoothing_alpha=ns.action_smoothing_alpha,  # 动作滤波系数。
        controller_mode=ns.controller_mode,  # 控制接口类型。
        joint_position_delta_scale=ns.joint_position_delta_scale,  # 位置增量控制每步目标增量。
        position_control_kp=ns.position_control_kp,  # 位置控制比例增益。
        position_control_kd=ns.position_control_kd,  # 位置控制阻尼增益。
        goal_conditioned=ns.goal_conditioned,  # 是否输出 goal-conditioned 观测。
        reward_mode=ns.reward_mode,  # 奖励模式。
        use_her=ns.use_her,  # 是否启用 HER。
        her_goal_selection_strategy=ns.her_goal_selection_strategy,  # HER relabel 策略。
        her_n_sampled_goal=ns.her_n_sampled_goal,  # HER 采样目标数量。
