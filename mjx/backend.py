"""Helpers for locating Playground and checking whether Warp can actually run."""
...
_WARP_INITIALIZED = False  # 避免训练脚本和 benchmark 重复初始化 Warp。
...
def ensure_warp_runtime() -> None:
    ...
    if _WARP_INITIALIZED:
        return
    wp.init()  # `mjx` 的 warp impl 只有在这里成功后才值得继续启动训练。
...
def detect_playground_root(explicit_root: str = "") -> str:
    ...
    if env_root:
        ...
        if (p / "learning").exists() and (p / "mujoco_playground").exists():
            return str(p)
    # 这些位置覆盖了“在 Playground 仓库里运行”和“放在 home 目录旁边运行”两种常见方式。
